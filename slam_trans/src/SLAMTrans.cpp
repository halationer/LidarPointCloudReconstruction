...
void SLAMTrans::HandlePointClouds(const sensor_msgs::PointCloud2 & vLaserData)
{
  ...
  if(!(m_iFrames%m_iSampleNum)){
    ...
    //Keep the original time and ignore the converted time
    vTransposedData.header.frame_id = m_sPointTargetFrame;
    //timestmap is the same with the input
    vTransposedData.header.stamp = vLaserData.header.stamp;
...
void SLAMTrans::HandleTrajectory(const nav_msgs::Odometry & oTrajectory)
{
  ...
  geometry_msgs::PoseStamped oOdomRaw;
  oOdomRaw.header = oTrajectory.header;
  oOdomRaw.header.stamp = oTrajectory.header.stamp;  
  oOdomRaw.pose = oTrajectory.pose.pose;


  geometry_msgs::PoseStamped oOdomNew;

  //Convert coordinates by TF
  try{
    
    m_pTFListener->transformPose(m_sOdomTargetFrame, oOdomRaw, oOdomNew);
  ...
  oTransposedOdom.pose.pose.position.x = oOdomNew.pose.position.x; 
  oTransposedOdom.pose.pose.position.y = oOdomNew.pose.position.y;
  oTransposedOdom.pose.pose.position.z = oOdomNew.pose.position.z - m_fOdomZOffset;
  oTransposedOdom.pose.pose.orientation = oOdomNew.pose.orientation;
