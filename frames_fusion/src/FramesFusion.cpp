...
FramesFusion::FramesFusion(ros::NodeHandle & node,
                       ros::NodeHandle & nodeHandle): m_oNodeHandle(nodeHandle), m_iOdomCount(0), m_dAverageReconstructTime(0), m_dMaxReconstructTime(0),  m_iReconstructFrameNum(0){
...
FramesFusion::~FramesFusion() {
	...
	<< "Final point nums: " << m_vMapPCN.size() + m_vMapPCNAdded.size()
...
void FramesFusion::HandlePointClouds(const sensor_msgs::PointCloud2 & vLaserData)
{
	...
	if(pFramePN->is_dense == false) {//标识码
		...
		// std::cout << m_vCurrentFramePoints.size() << std::endl;
...
void FramesFusion::SurfelFusion(pcl::PointNormal oLidarPos, pcl::PointCloud<pcl::PointNormal>& vDepthMeasurementCloud) {
	...
	std::cout << std::format_blue << "new point num: " << vDepthMeasurementCloud.size() << std::format_white << std::endl;
