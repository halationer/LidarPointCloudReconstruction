...
public:
	...
	//transform pcl::PointXYZI and faceparam to pcl::pointnormal
	void ChangeNormalType(const pcl::PointCloud<pcl::PointXYZI> & vClouds, const std::vector<FacePara> & vFaceParams, pcl::PointCloud<pcl::PointNormal> & vPNormals);
	...
	pcl::Normal CaculateTriangleNormal(pcl::PointXYZI & oPZero, pcl::PointXYZI & oPOne, pcl::PointXYZI & oPTwo);
	//reload, caculate triangle face normal with a reference point
	void CaculateTriangleNormal(const pcl::PointXYZI & oPZero, const pcl::PointXYZI & oPOne, const pcl::PointXYZI & oPTwo, 
		const pcl::PointXYZI & oInnerP, FacePara & oFacePara);
	//reload, caculate triangle face normal with a reference point
	void CaculateTriangleNormal(const pcl::PointCloud<pcl::PointXYZI> & vVertices,const pcl::PointXYZI & oInnerP, 
	...
	void ComputeCenterPoint(const pcl::PointCloud<pcl::PointXYZI> & vVertices, const std::vector<pcl::Vertices> & vMeshVertexIdxs,
		                    pcl::PointCloud<pcl::PointXYZI> & vCenterPoints);
	//reload, compute the center point based on ALL given triangle vertices
	void ComputeCenterPoint(const pcl::PointCloud<pcl::PointXYZI> & vVertices, pcl::PointXYZI & oCenterPoint);


	//Calculate equation parameters for all faces with a given center point (reference)
	void ComputeAllFaceParams(const pcl::PointCloud<pcl::PointXYZI> & vVertices, const std::vector<pcl::Vertices> & vMeshVertexIdxs, Eigen::MatrixXf & oMatNormal, Eigen::VectorXf & vfDParam);
	//Calculate equation parameters for all faces without a given base point (reference)
	void ComputeAllFaceParams(const pcl::PointCloud<pcl::PointXYZI> & vVertices, const std::vector<pcl::Vertices> & vMeshVertexIdxs, pcl::PointXYZI & oCenterPoint,
	...
	void ComputeAllFaceParams(const pcl::PointXYZI & oViewPoint, const pcl::PointCloud<pcl::PointXYZI> & vVertices, std::vector<pcl::Vertices> & vMeshVertexIdxs,
		                      Eigen::MatrixXf & oMatNormal, Eigen::VectorXf & vfDParam);
	//Calculate right direction normal for all faces correspoding to a given viewpoint (reference)
	void ComputeAllFaceParams(const pcl::PointXYZI & oViewPoint, const pcl::PointCloud<pcl::PointNormal> & vPNormals,
	...
	void FaceParamInTwoSpace(const pcl::PointCloud<pcl::PointXYZI> & vConvertVertices, const std::vector<pcl::Vertices> & vConvertMeshIdxs,
		                     const pcl::PointCloud<pcl::PointXYZI> & vEuclidVertices, const std::vector<pcl::Vertices> & vEuclidMeshIdxs);
	...
	std::vector<SignedDis> ComputePointSignedDis(pcl::PointCloud<pcl::PointXYZI> & vQueryCloud);
	...
	std::vector<float> ComputePointSignedDis(const std::vector<SignedDis> & vConvexSD, const pcl::PointCloud<pcl::PointXYZI> & vQueryCloud,
	...
	std::vector<float> ComputePointSignedDis(const pcl::PointCloud<pcl::PointXYZI> & vQueryCloud, const Eigen::MatrixXf & m_oMatN,const Eigen::VectorXf & m_vD);
...
//
//
//	pcl::PointCloud<pcl::PointXYZI>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZI>);//ԭʼpcd����
//	pcl::PointCloud<pcl::PointXYZI>::Ptr pQuerycloud(new pcl::PointCloud<pcl::PointXYZI>);
//	std::vector<Point3D> point3d;//Point3DĿ��������������
//	//��ȡ��������
//	HPDpointclouddataread("bunny.las", cloud, point3d, 1);
//	std::vector<Point3D> oripoint3d(point3d);
//	//�����ӵ�;
//	pcl::PointXYZI oViewPoint;
...
//	//center of each face
//	pcl::PointCloud<pcl::PointXYZI>::Ptr pCenterCloud(new pcl::PointCloud<pcl::PointXYZI>);//ԭʼpcd����
...
//	viewer->addPolygonMesh<pcl::PointXYZI>(hpdhpr.m_pHullVertices, hpdhpr.m_vHullPolygonIdxs, "polyline");
//	viewer->addPointCloudNormals<pcl::PointXYZI, pcl::Normal>(pCenterCloud, pFaceNormal, 1, 50.0f, "normal");
